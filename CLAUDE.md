...
- **ה. Zero-Click Data Extraction & Call Summarization** (תמלול שיחה קולית + סיכום + חילוץ שדות אוטומטי + פנייה ב-WhatsApp להשלמת פרטים חסרים) - ⚠️ **בנוי חלקית בלבד, במכוון**: Click-to-Call (`voice_call.py`) מקליט + הנציג מקליד **הערות ידניות** → Claude מסכם - **לא** תמלול אוטומטי מההקלטה עצמה, ו**אין** חילוץ שדות אוטומטי מהשיחה, ו**אין** פנייה אוטומטית ב-WhatsApp על שדות חסרים. זו החלטה מכוונת מ-2026-08-27 (המשתמש נשאל ובחר "הקלטה + הערות ידניות" על פני תמלול, כי אין ספק STT מוגדר ועברית-איכותית דורשת ספק כמו Whisper - עלות/תלות חדשה) - **לא שכחה**, אבל אם רוצים לסגור את הפער הזה לחזון המלא, זה ידרוש ספק STT (למשל OpenAI Whisper) + לוגיקת חילוץ שדות (בדומה ל-`extract_customer_info` הקיים, אבל מתמלול שיחה ולא מהודעת טקסט) + טריגר WhatsApp על שדות חסרים (לא קיים היום בשום מקום בקוד).

**פערים נוספים בין החזון לקוד הקיים (מוזכרים מפורשות בהודעת המשתמש, לא רק במסמך):**
- **מנגנון חיוג רציף (sequential/power dialer)** - **לא בנוי בכלל**. Click-to-Call היום הוא ליד-אחד-בכל-פעם (גישור ידני מהדשבורד), לא חיוג אוטומטי רציף דרך רשימת לידים.
- **ארכיטקטורת הפעולה המלאה** (סעיף 3 במסמך): שלב 4 - "פנייה אוטומטית ב-WhatsApp להשלמת פרטים חסרים" - **לא קיים**. אין היום שום לוגיקה שמזהה "שדה חסר בכרטיס" ויוזמת הודעה לליד לבקש אותו.
